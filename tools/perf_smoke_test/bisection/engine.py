...
def _component_stack_diff(from_sha: str, to_sha: str, *, relation: str) -> dict | None:
    ...
    """
    try:
        from_stack = resolve_stack(_REPO_ROOT, from_sha)
        to_stack = resolve_stack(_REPO_ROOT, to_sha)
...
def _build_stack_diff(
    *, good_ref: str, bad_ref: str, last_good: str | None = None, first_bad: str | None = None
) -> dict | None:
    ...
    """
    diff: dict[str, dict] = {}
    range_diff = _component_stack_diff(good_ref, bad_ref, relation="good_ref_to_bad_ref")
    if range_diff is not None:
        diff["range"] = range_diff
    if last_good and first_bad and last_good != first_bad:
        culprit = _component_stack_diff(last_good, first_bad, relation="last_good_to_first_bad")
...
def _run_single_measurement(
    ...
    *,
    ...
    command = format_runner_command(effective_plan, output_dir, commit_sha, artifact_dir, knobs=knobs)
...
def _run_warmup_measurements(
    ...
    *,
    ...
    return run_warmups(
        ...
        policy=policy,
        probe_policy=probe_policy,
        measure_with_recovery=_measure_with_recovery,
    )
...
def _measure_reference_commit(
    ...
    *,
    ...
    warmup_failure = _run_warmup_measurements(
        plan, output_dir, commit_sha=commit_sha, label=label, policy=policy, probe_policy=probe_policy
    )
...
def run_local_candidate(
    ...
    *,
    ...
    warmup_failure = _run_warmup_measurements(
        plan, output_dir, commit_sha=commit_sha, label="candidate", policy=policy, probe_policy=probe_policy
    )
    if warmup_failure is not None:
    ...
    *,
    ...
    if not reference_check.reproduced:
        ...
        summary = BisectionSummary(
            ...
            reference_stats=reference_stats,
            non_repro=non_repro,
            stack_diff=_build_stack_diff(good_ref=good_sha, bad_ref=bad_sha),
        )
        _write_summary(output_dir, plan, summary)
        return summary
